...
// Generates a clean, vector PDF purchase order using jsPDF — styled to match
// exportInvoicePDF.js so PDFs feel consistent across the app.
//
// Install:  npm install jspdf (already installed for invoices)
// Usage:    import { exportPurchaseOrderPDF } from '../utils/exportPurchaseOrderPDF'
//           await exportPurchaseOrderPDF(po, vendor, items, orgId)
...
export async function exportPurchaseOrderPDF(po, vendor, items = [], orgId) {
  if ((!items || items.length === 0) && po?.id) {
    const { data: fetchedItems, error: itemsErr } = await supabase
      .from('purchase_order_items')
      .select('*')
      .eq('po_id', po.id)
      .eq('org_id', orgId)
    if (!itemsErr && Array.isArray(fetchedItems)) items = fetchedItems
  }
  ...
  doc.setFontSize(18)
  setColor(doc, C.text)
  doc.text('PURCHASE ORDER', pw - mr, 16, { align: 'right' })

  doc.setFont('helvetica', 'normal')
  doc.setFontSize(10)
  setColor(doc, C.muted)
  doc.text('PO No.', pw - mr, 23, { align: 'right' })
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(11)
  setColor(doc, C.teal)
  doc.text(po.number || '—', pw - mr, 29, { align: 'right' })
  ...
  doc.setFontSize(7)
  setColor(doc, C.teal)
  doc.text('VENDOR', ml + cw * 0.42, ry)

  ry += 5
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(10)
  setColor(doc, C.text)
  doc.text(vendor?.name || '—', ml + cw * 0.42, ry)

  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8.5)
  setColor(doc, C.muted)
  if (vendor?.email)   { ry += 4.5; doc.text(vendor.email, ml + cw * 0.42, ry) }
  if (vendor?.phone)   { ry += 4;   doc.text(vendor.phone, ml + cw * 0.42, ry) }
  if (vendor?.address) { ry += 4;   doc.text(vendor.address, ml + cw * 0.42, ry) }

  const cityLine = [vendor?.city, vendor?.province, vendor?.postal_code].filter(Boolean).join(', ')
  ...
  let dy = headerH + 10
  const dateRows = [
    ['Order Date', fmtDate(po.date)],
    ['Expected',   po.expected_date ? fmtDate(po.expected_date) : '—'],
  ]
  ...
  if (po.notes && po.notes.trim() !== '') {
    ...
    doc.setFontSize(8.5)
    setColor(doc, C.muted)
    const lines = doc.splitTextToSize(po.notes, pw - ml - mr)
    doc.text(lines, ml, y)
    y += lines.length * 4
  }
  ...
  doc.setFontSize(8)
  setColor(doc, C.light)
  doc.text('Please confirm receipt of this purchase order.', pw / 2, footerY + 5, { align: 'center' })
  doc.text(COMPANY.name + '  ·  ' + COMPANY.phone, pw / 2, footerY + 9.5, { align: 'center' })

  const filename = `${po.number || 'purchase-order'}-${vendor?.name?.replace(/\s+/g, '-') || 'PO'}.pdf`
  doc.save(filename)
